...
void CDownLoadNowNewsAVDlg::OnTimer(UINT nIDEvent) 
{
	...
	CString StrConvertWMV,StrWMVFileName,StrJPGFileName,StrJPGFileName1;
	CString StrCutImage,StrCutImage1;
	CString StrDelFile;
	...
	if(blntodayfirst)
	{
		...
		for(i=0;i<j;i++)
		{
			...
			if(StrBuf1!="")
			{
				...
				StrBuf.Replace("192.168.0.200", "61.67.114.50");//透過真實IP抓資料 by jash modify at 20140709
				...
				StrWMVFileName+=".wmv";
				StrJPGFileName=StrBuf3.Mid(0,(StrBuf3.GetLength()-4));
				StrJPGFileName1=StrJPGFileName;
				
				StrJPGFileName+=".jpg";
				StrJPGFileName1+="_0.jpg";
				
				StrConvertWMV.Format("cscript.exe wmcmd.vbs -input %s -output %s -loadprofile wmv_1m.prx",StrBuf3,StrWMVFileName);
				StrCutImage.Format("ffmpeg -i %s -y -f  image2 -s 160X120 -ss 20 -vframes 1  %s",StrWMVFileName,StrJPGFileName);
				StrCutImage1.Format("ffmpeg -i %s -y -f  image2 -s 214X160 -ss 20 -vframes 1  %s",StrWMVFileName,StrJPGFileName1);
				StrDelFile="Del "+StrBuf3;
				
				pf2=fopen("WgetFtp2Wmv.bat","w");
				fprintf(pf2,"%s\n",StrBuf.GetBuffer(0));
				fprintf(pf2,"%s\n",StrConvertWMV.GetBuffer(0));
				fprintf(pf2,"%s\n",StrCutImage.GetBuffer(0));
				fprintf(pf2,"%s\n",StrCutImage1.GetBuffer(0));
