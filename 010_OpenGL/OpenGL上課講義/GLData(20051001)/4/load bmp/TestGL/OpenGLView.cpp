...
{
	SetObjXYZRange(10,-10,10,-10,10,-10);
	GLHardAccel=FALSE;
	m_pDC=NULL;
	m_hRC=NULL;
...
void OpenGLView::CreateProjMatrix(double xmax, double xmin, double ymax, double ymin)
{
    ...
    wglMakeCurrent(NULL,NULL);
	ImageViewScaleX=(float)(ImagePixelScaleX*ScrWidth/(EyeXMax-EyeXMin));
	ImageViewScaleY=(float)(ImagePixelScaleY*ScrHeight/(EyeYMax-EyeYMin));
}
