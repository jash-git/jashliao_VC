...
int CTestGLView::SaveDIBitmap(CArchive& ar)
{
	int width,height;
    ...
    info.biSizeImage = NbBytes; 
    info.biSize = sizeof(BITMAPINFOHEADER); 
    info.biPlanes = 1; 
    info.biBitCount =  3 * 8; // RGB 
    info.biCompression = 0; 
    info.biXPelsPerMeter = 0; 
    info.biYPelsPerMeter = 0; 
    info.biClrUsed = 0; 
    info.biClrImportant = 0; 
	header.bfType=0x4d42;   // 'BM' WINDOWS_BITMAP_SIGNATURE
    header.bfOffBits=sizeof(BITMAPFILEHEADER)+info.biSize;
	header.bfSize=NbBytes+header.bfOffBits;
	header.bfReserved1=0;
	header.bfReserved2=0;

	ar.Write(&header,sizeof(BITMAPFILEHEADER));
	ar.Write(&info,sizeof(BITMAPINFOHEADER));
	ar.Write(pPixelData,NbBytes);
/*
	FILE *fp;
