...
void CCheck5TVEncoderChannelDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if(m_intTimerCount>=20)
	{
		...
		const size_t strsize02=(m_StrMMSName.GetLength()+1)*2;
		const size_t strsize03=(m_StrMMSUrl.GetLength()+1)*2;
		wcstombs_s(&sz01,pstr01,strsize01,m_StrID,_TRUNCATE);//CString2char
		//////////////////////////////////////////////////////////////////////
		//UTF8 to ANSI
		DWORD dwNum = WideCharToMultiByte(CP_ACP, NULL, m_StrMMSName, -1, NULL, 0, NULL, FALSE);
		pstr02=new char [dwNum];
		WideCharToMultiByte(CP_ACP, NULL, m_StrMMSName, m_StrMMSName.GetLength(), pstr02, dwNum, NULL, FALSE);
		pstr02[dwNum-1]= '\0';
		///////////////////////////////////////////////////////////////////////
		wcstombs_s(&sz03,pstr03,strsize03,m_StrMMSUrl,_TRUNCATE);//CString2char
		pflog=fopen("Playlog.csv","a");//test
		int intServiceID=atoi(pstr01);
		if(m_blnState==true)
		{
			fprintf(pflog,"%s,%s,%s,%d sec\n",pstr01,pstr02,pstr03,m_intSec);//test
			//WebServiceUpdate(intServiceID,0,m_intSec);
		}
		else
		{
			fprintf(pflog,"%s,%s,%s,%d sec\n",pstr01,pstr02,pstr03,-1);//test
