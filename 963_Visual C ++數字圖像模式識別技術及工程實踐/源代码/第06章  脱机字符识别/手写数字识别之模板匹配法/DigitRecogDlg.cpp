...
void CDigitRecogDlg::OnButtonInputTest() 
{
	...
	POSITION pos = FileDlg.GetStartPosition();//Returns the position of the first element of the filename list.
	...
	if((iTotalTestNum = arTestSetFilePath.GetSize()) > 0){
		iCurTestNum = 0;
		strFile = arTestSetFilePath.GetAt(0);//显示第0个文件的特征，然后进行识别
...
void CDigitRecogDlg::OnButtonInputTrain() 
{
	...
	pEditResult->SetSel(0,-1);//选中文本框中的所有东西
	pEditResult->ReplaceSel("");//以空替换上面所选中的
...
void CDigitRecogDlg::OnButtonSaveTemplate() 
{
	// TODO: Add your control notification handler code here
	CFile cf;
	if(cf.Open("D:\\mydata.dat",CFile::modeCreate|CFile::modeNoTruncate|CFile::modeReadWrite)==NULL)
	{
