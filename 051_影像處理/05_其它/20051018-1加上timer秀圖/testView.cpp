...
BEGIN_MESSAGE_MAP(CTestView, CView)
	...
	ON_WM_SIZE()
	ON_COMMAND(ID_TimerShow, OnTimerShow)
	ON_WM_TIMER()
	//}}AFX_MSG_MAP
...
void CTestView::Onchangegray() //灰階
{
	...
	testDib3.Create(Width,Height,24);	
	...
	for(x=0;x<Width;x++)
	{
		for(y=0;y<Height;y++)
		{
			DWORD *q=(DWORD *)testDib3.GetBits(x,y);//利用物件本身所提供之成員函數取得一個像素的 R,G,B(指標用法)
			*q=dc.GetPixel(x,y); 
		}
	}
	testDib3.SaveBmp("test.bmp"); 
	ftime(&t1);
...
void CTestView::OnTimer(UINT nIDEvent) 
{
	...
	if(count<=2)
	{
		testDib.LoadBmp(strname[count].GetBuffer(0));
		count++;
	}
	else
	{
