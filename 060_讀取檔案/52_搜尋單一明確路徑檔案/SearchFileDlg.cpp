...
void CSearchFileDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_FILES, m_ctrlFilesList);
...
BEGIN_MESSAGE_MAP(CSearchFileDlg, CDialog)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(ID_BUTTON_SEARCHFILE, OnSearchfile)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CSearchFileDlg::OnSearchfile() 
{
	CString StrBuf;
	UpdateData(TRUE);
	StrBuf=m_strDir+m_strFile;
	if(SearchFile(StrBuf))
	{
		MessageBox("find one");
	}
	else
...
BOOL CSearchFileDlg::SearchFile(CString StrFileName)
{
	CFileFind ff;	
