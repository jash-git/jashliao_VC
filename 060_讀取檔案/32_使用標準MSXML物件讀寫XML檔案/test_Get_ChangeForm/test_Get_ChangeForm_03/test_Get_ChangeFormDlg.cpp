...
void CTest_Get_ChangeFormDlg::OnButton3() 
{
	// TODO: Add your control notification handler code here
	MSXML2::IXMLDOMDocumentPtr pDoc;
	HRESULT hr = pDoc.CreateInstance(__uuidof(MSXML2::DOMDocument30));
	...
	//在樹中查找名為Book的節點,"//"表示在任意一層查找 
	pNode=pDoc->selectSingleNode("//Default");
	MSXML2::DOMNodeType nodeType; 
	...
	variantValue=(_bstr_t)m_strId;
	pAttrItem->put_nodeTypedValue(variantValue);
	pDoc->save("SignageConfig.xml");
	MessageBox("OK");
}

void CTest_Get_ChangeFormDlg::OnButton4() 
{
	// TODO: Add your control notification handler code here
	MSXML2::IXMLDOMDocumentPtr pDoc;
	HRESULT hr = pDoc.CreateInstance(__uuidof(MSXML2::DOMDocument30));
	...
	if(pNode!=NULL)
	{
		...
		file1.Open("changeform.dat",CFile::modeCreate|CFile::modeWrite);
