...
FILE *pfNotCopy;//無法複製FileName與UP
FILE *pfCopyOK;//紀錄複製成功資料
/////////////////////////////////////////////////////////////////////////////
// CAboutDlg dialog used for App About
CString gStrDirValue;
...
void CNetWork_Browse_CopyDlg::OnButton3() //執行檔案名稱儲存到記憶體
{
	...
	pfNotCopy=fopen("NotCopy.txt","w");//無法複製FileName與UP
	/*
	int i=0;
	char chrSeps='\\';//建立分割符號
			...
			switch(intTimer2Step)
			{
				...
				case 5://判斷所有IP是否做完
					...
					else
					{
						...
						fclose(pfNotCopy);//無法刪除FileName與UP
						fclose(pfCopyOK);//紀錄刪除成功資料
						KillTimer(2);
...
void CNetWork_Browse_CopyDlg::CreateNetWorkPath()//產生連線網址
{
	...
	pNode=pDoc->selectSingleNode("//nabro");
	...
	StrNetWorkDir="\\\\";
	StrNetWorkDir+=StrIP;
	StrNetWorkDir+="\\d$\\NabroClient\\";
	StrNetWorkDir+=strValue;
	m_StrNetWorkDir=StrNetWorkDir;
	UpdateData(false);
}
...
bool CNetWork_Browse_CopyDlg::MountNetWorkDir(int intTimer)//建立網路磁碟機
{
	...
	//在樹中查找名為Book的節點,"//"表示在任意一層查找 
	pNode=pDoc->selectSingleNode("//NetWork");
	MSXML2::DOMNodeType nodeType; 
	//得到節點類型 
	pNode->get_nodeType(&nodeType); 
	...
	_variant_t vtStr;
	pNode->get_nodeTypedValue(&vtStr);  
	gStrDirValue=vtStr.bstrVal; 
	//MessageBox(strValue);
	//MessageBox("OK");
	//////////////////////////////////////////////////
	//strcpy(drv,"X:");
	strcpy(drv,(_bstr_t)vtStr);
	NETRESOURCE netres;
	netres.dwScope=RESOURCE_GLOBALNET;
	netres.dwType=RESOURCETYPE_ANY;
	netres.dwDisplayType=RESOURCEDISPLAYTYPE_GENERIC;
...
void CNetWork_Browse_CopyDlg::Timer1Function()
{
	...
	{//無檔案
		...
		if(CopyFile(StrDir.GetBuffer(0),StrNetWorkFile.GetBuffer(0),FALSE))
		{
			...
			this->SetDlgItemText(IDC_EDIT6,"此筆檔案複製中...");
		}
		else
		{
			MessageBox("檔案複製失敗");
	...
	if(intShowCount==m_FileAmount)
	{
		...
		but1->EnableWindow(true);
		char drv[100];
		strcpy(drv,gStrDirValue.GetBuffer(0));
		DWORD rslt=WNetCancelConnection2(drv, 0, FALSE);
		if(rslt!=0)
			MessageBox("無法中斷網路磁碟機");
		MessageBox("完成");
...
void CNetWork_Browse_CopyDlg::OnButton4()//自動檔案刪除 
{
	...
	pfNotConnect=fopen("NotConnect.txt","w");//無連線IP
	pfNotCopy=fopen("NotCopy.txt","w");//無法複製FileName與UP
	pfCopyOK=fopen("CopyOK.txt","w");//紀錄複製成功資料
	SetTimer(2,1000,NULL);//啟動Timer2
}
bool CNetWork_Browse_CopyDlg::AutomaticCopyFile()
{
	CString StrDir,StrFileName;//UI變數
	CString StrState,StrBuf;//UI變數
	CString StrNetWorkDir;
	...
	{//無檔案
		...
		{
			m_IPData.GetWindowText(StrIP); 
			fprintf(pfCopyOK,"%s\t%s\n",StrIP.GetBuffer(0),StrFileName.GetBuffer(0));//紀錄複製成功資料
			this->SetDlgItemText(IDC_EDIT6,"此筆檔案複製中...");
		}
		else
		{
			//MessageBox("檔案刪除失敗");
			m_IPData.GetWindowText(StrIP); 
			fprintf(pfNotCopy,"%s\t%s\n",StrIP.GetBuffer(0),StrFileName.GetBuffer(0));//無法複製FileName與UP
		}
	}
	//////////////////////////////////////////
	intShowCount++;
