...
FILE *pfOKConnect;//可連線IP
/////////////////////////////////////////////////////////////////////////////
bool blnChecktime=false;//定時
//////////////////////////////////////////////////////////////////////////////
...
void CNetWork_Browse_CopyDlg::OnTimer(UINT nIDEvent)//真正執行的主程式 
{
	...
	*/
	bool return1;
	CButton *but1;
	int intYear,intMonth,intDay,intHours;
	...
	switch(nIDEvent)
	{
		case 1:
			...
			break;
		case 2:
			if(blnChecktime==false)
			{
				intYear=this->GetDlgItemInt(IDC_year); 
				intMonth=this->GetDlgItemInt(IDC_month);
				intDay=this->GetDlgItemInt(IDC_day);
				intHours=this->GetDlgItemInt(IDC_hours);
				m_nYear = time.GetYear();      ///年
				m_nMonth = time.GetMonth();      ///月
				m_nDay = time.GetDay();      ///日
				m_nHour = time.GetHour();      ///時
				if(m_nYear==intYear && m_nMonth==intMonth && m_nDay==intDay && m_nHour==intHours)
				{
					blnChecktime=true;
				}
				else
				{
					this->SetDlgItemText(IDC_EDIT6,"等待執行...");
					blnChecktime=false;
				}
			}
			else
...
void CNetWork_Browse_CopyDlg::OnButton4()//自動檔案刪除 
{
	...
	pfOKConnect=fopen("OKConnect.txt","w");//可連線IP
	SetTimer(2,1000,NULL);//啟動Timer2
}
...
void CNetWork_Browse_CopyDlg::OnClose() 
{
	// TODO: Add your message handler code here and/or call default
	char drv[100];
	strcpy(drv,"X:");
...
void CNetWork_Browse_CopyDlg::OnButton5() 
{
	// TODO: Add your control notification handler code here
