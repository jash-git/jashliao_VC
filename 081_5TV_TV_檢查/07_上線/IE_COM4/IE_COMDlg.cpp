...
BOOL CIE_COMDlg::OnInitDialog()
{
	...
	DeleteFile("text.txt");
	COleVariant vaUrl="http://5tv.tw-airnet.net/810a/db/module/mms_view/get_all_mms.php?type=tv&online=1";
...
//*/
	...
	char fg[700];
	int intlen;
	a1.open("text.txt",ios::in);
	pf=NULL;
	pf=fopen("data.txt","w");
	CString StrDataBuf,StrDataBuf1;
	while(!a1.eof())
	{
		a1.getline(fg,500);
		StrDataBuf=fg;
		intlen=StrDataBuf.GetLength();
		if(intlen > 5)
		{
			StrDataBuf1=StrDataBuf.Mid(0,11);
			if(StrDataBuf1=="<BODY><PRE>")
			{
				StrDataBuf1=StrDataBuf.Mid(11);
			}
			else
			{
				StrDataBuf1=StrDataBuf;
			}
			if(StrDataBuf1 != "</PRE></BODY>")
			{
				fprintf(pf,"%s\n",StrDataBuf1.GetBuffer(0)); 
			}
		}
	}
	a1.close(); 
	...
	///////////////////////////////6_end
	Sleep(5000);
	ShellExecute(NULL,"open","wmp002.exe",NULL,NULL,SW_SHOWNORMAL);
	this->OnOK(); 	
}
///////////////////////////5_start
