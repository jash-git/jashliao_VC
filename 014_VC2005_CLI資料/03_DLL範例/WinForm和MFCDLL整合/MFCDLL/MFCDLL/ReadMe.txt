...
AppWizard 已經為您建立了這個 MFCDLL DLL。這個 DLL 不僅
示範了使用 MFC 的基本概念，也是您撰寫 DLL 的起點。

這個檔案包含一份摘要，簡要說明構成 MFCDLL DLL 的
所有檔案，它們個別的內容。
...
MFCDLL.h
    這是 DLL 的主要標頭檔。它宣告了 CMFCDLLApp 類別。

MFCDLL.cpp
    這是主要 DLL 原始程式檔。它含有 CMFCDLLApp 類別。
...
StdAfx.h, StdAfx.cpp
    這些檔案是用來建置名為 MFCDLL.pch 的先行編譯標頭 (PCH) 檔，
