...
*/
	...
	if(strUrl=="file:///C:/index.htm") 
	{
		CComPtr<IDispatch>pDisp;
		if(HasItem(objAllElement,"frm_test")==true) //voteform為投票選項所在的Form
		{
			objAllElement->item(COleVariant("frm_test"),COleVariant((long)0),&pDisp);
			...
			str[0]="jashliao";
			str[1]="C:\\123.txt";
			PutIEText(objAllElement,"T1",str[0]);
			PutIEFile(objAllElement,"F1",str[1]);
...
void CIE_COMDlg::PutIEFile(IHTMLElementCollection *pobjAllElement,CString strName,CString strText)
{
	CComPtr<IDispatch> pDisp;
	pobjAllElement->item(COleVariant(strName),COleVariant((long)0),&pDisp);
...
void CIE_COMDlg::OnTimer(UINT nIDEvent) 
{
	...
	EmptyClipboard();
	HGLOBAL hData;
	hData=::GlobalAlloc(GMEM_DDESHARE,Strdata.GetLength()+1);
	char *pchrData;
	pchrData=(char *)::GlobalLock(hData);
	strcpy(pchrData,LPCSTR(Strdata));
	::GlobalUnlock(hData);
	...
	char chb=18;//Alt
	cha='O';
	keybd_event(chb, 0, 0 ,0);
	keybd_event(cha, 0, 0 ,0);
	//Sleep(1000);
	keybd_event(cha, 0, KEYEVENTF_KEYUP ,0);
	keybd_event(chb, 0, KEYEVENTF_KEYUP ,0);	
