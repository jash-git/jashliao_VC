...
//資料陣列指標；資料陣列屬於第幾個lngBlock；陣列大小
{
	...
	for(i=0;i<n;i++)
	{
		...
		if(lngtype==1)//Line
		{
			...
			gInputData.CCW=2-lngDirection;//紀錄方向，0->右；1->左
			gInputData.Direction=(gEE+i)->CCW;//紀錄順逆，0->順；1->逆
...
long EXPORTED_DLL_FUNCTION ArcParagraph(CArc &data,const double tolerance)
{
	...
	if(number<=1)
	{
		if(number==1)
		{
			...
			Point3DAns[0].m_x=Center.m_x+R*cos((EndAngle)); 
			Point3DAns[0].m_y=Center.m_y+R*sin((EndAngle)); 
			Point3DAns[0].m_z=Center.m_z+R*0;
			return 1;
		}
		else
		{
...
long EXPORTED_DLL_FUNCTION LineParagraph(CLine &data,const double tolerance)
{
	...
	if(number<=1)
	{
		...
		{
			Point3DAns=new CPoint3D[number];
			Point3DAns[0].m_x=data.m_EndPoint.m_x; 
			Point3DAns[0].m_y=data.m_EndPoint.m_y; 
			Point3DAns[0].m_z=data.m_EndPoint.m_z;
			return 1;
		}
		else
		{
