================================================================================
    MFC 程式庫 : STBMMSConnect 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 STBMMSConnect 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 STBMMSConnect 應用程式的檔案。

STBMMSConnect.vcproj
...
STBMMSConnect.h
    ...
    並宣告 CSTBMMSConnectApp 應用程式類別。

STBMMSConnect.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CSTBMMSConnectApp。
...
res\STBMMSConnect.ico
    這個圖示檔是用來做為應用程式的圖示。這個圖示包含在主要資源檔
    STBMMSConnect.rc 內。

res\STBMMSConnect.rc2
...
STBMMSConnectDlg.h、STBMMSConnectDlg.cpp - 對話方塊
    這些檔案包含您的 CSTBMMSConnectDlg 類別。這個類別定義應用程式主
    對話方塊的行為。對話方塊的範本在 STBMMSConnect.rc 內，您可以 
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 STBMMSConnect.pch 的先行編譯標頭 (PCH) 檔，
