...
void CSTBMMSConnectDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDITMMS, m_V1);
	DDX_Text(pDX, IDC_EDITSTB, m_V2);
	DDX_Text(pDX, IDC_EDITMSG, m_V3);
}
...
BEGIN_MESSAGE_MAP(CSTBMMSConnectDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTShowMMS, &CSTBMMSConnectDlg::OnBnClickedButtshowmms)
	ON_BN_CLICKED(IDC_BUTTShowSTB, &CSTBMMSConnectDlg::OnBnClickedButtshowstb)
END_MESSAGE_MAP()
...
void CSTBMMSConnectDlg::SendMSG()
{
	...
	for(i=0;i<j;i++)
	{
		...
		if(ConnectSTB(StrIP))
		{
			...
			StrSourceFileName=_T("mmsconnectCmd.Msg");
...
void CSTBMMSConnectDlg::OnBnClickedButtshowmms()
{
	InitVariable();
	//**
	GetARPTable();
	...
	//**
	UpdateData(true);
	m_V1=m_ArrayARPIP.GetSize();
	UpdateData(false);
}

void CSTBMMSConnectDlg::OnBnClickedButtshowstb()
{
	int i,j;
	m_ArrayCheckIP.RemoveAll();//CArray Step_03
	j=STBListBox.GetCount();
	...
	//**
	UpdateData(true);
	m_V2.Format(_T("%d"),m_ArrayCheckIP.GetSize());
	UpdateData(false);
	//**
	m_ArrayMessageIP.RemoveAll();//CArray Step_03
	j=MSGListBox.GetCount();
	...
	//**
	UpdateData(true);
	m_V3.Format(_T("%d"),m_ArrayMessageIP.GetSize());
	UpdateData(false);
}
