...
BOOL CIE_COMDlg::OnInitDialog()
{
	...
	b.open("proxy.txt",ios::in);
	while(!b.eof())
	{
		b.getline(fg,5000);
		StrBuf=fg;
		m_proxyArray.Add(StrBuf);
	}
	gintSize=m_proxyArray.GetSize();
	Rand_One2N(gintSize,0);
	StrBuf=m_proxyArray.GetAt(gintindex);
	//SetConnectionProxy(StrBuf.GetBuffer(0));
	ShellExecute(NULL,"open","SetProxy.exe",StrBuf.GetBuffer(0),NULL,SW_SHOWNORMAL);
