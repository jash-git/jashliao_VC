...
char *gchrbuf;
char gchrFileName[100], gchrStartDay[100], gchrEndDate[100];
long glngStartDay;
long glngEndDay;
/////////////////////////////////////////////////////////////////////////////
...
bool CMultiThreadFTPDlg::isNeedFile00(CString StrData)
{
	long lngnumber;
	char *chrdata;
	CString StrBuf;
	StrBuf=StrData.Mid(4,8);
	chrdata=StrBuf.GetBuffer(0);
	lngnumber=atol(chrdata);
	if((lngnumber >= glngStartDay) && (lngnumber <= glngEndDay))
	{
		return true;
	}
	else
...
bool CMultiThreadFTPDlg::isNeedFile01(CString StrData)
{
	long lngnumber;
	char *chrdata;
	CString StrBuf;
...
bool CMultiThreadFTPDlg::isNeedFile02(CString StrData)
{
	long lngnumber;
	char *chrdata;
	CString StrBuf;
...
bool CMultiThreadFTPDlg::isNeedFile03(CString StrData)
{
	long lngnumber;
	char *chrdata;
	CString StrBuf;
...
bool CMultiThreadFTPDlg::isNeedFile04(CString StrData)
{
	long lngnumber;
	char *chrdata;
	CString StrBuf;
...
void CMultiThreadFTPDlg::Thread00Fun()
{
	...
	Session.SetOption(INTERNET_OPTION_CONNECT_TIMEOUT,1000*600);
...
void CMultiThreadFTPDlg::ReadListFile()
{
	...
	CString StrBuf;
	m_intSum=0;
	gchrbuf=sCmdLine.GetBuffer(0);
	int result = sscanf(gchrbuf, "%[^','],%[^','],%s",gchrFileName,gchrStartDay,gchrEndDate);
	//MessageBox(gchrFileName);
	//MessageBox(gchrStartDay);
	//MessageBox(gchrEndDate);
	glngStartDay=atol(gchrStartDay);
	glngEndDay=atol(gchrEndDate);
	if(sCmdLine=="")
	{
