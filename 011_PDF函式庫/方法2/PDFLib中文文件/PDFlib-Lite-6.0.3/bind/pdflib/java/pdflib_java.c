...
/* $Id: pdflib_java.c,v 1.94.2.11 2005/11/18 11:14:10 rjs Exp $
 *
...
#define PDF_JAVA_SAFE_INIT_VOID(j,p)                               \
    /* validate the JNIEnv first */                                \
    if (PDF_ISNULL(j)) {                                           \
        ...
        return;                                                    \
                                                                   ...
                                                                   \
    /* now get our p pointer from the JNIEnv pointer ... */        \
    p = PDF_ENV2PTR(j);                                            \
                                                                   \
    /* ... and validate it with PDF_check_context */               \
    if (!PDF_check_context(p)) {                                   \
        jthrow(jenv, "Invalid PDFlib object", 0, "pdflib_java.c"); \
        return;                                                    \
    }

#define PDF_JAVA_SAFE_INIT(j,p)                                    \
    /* validate the JNIEnv first */                                \
    if (PDF_ISNULL(j)) {                                           \
        jthrow(jenv, "Must not call any PDFlib method after delete()",\
            0, "pdflib_java.c");                                   \
    ...
    /* ... and validate it with PDF_check_context */               \
    if (!PDF_check_context(p)) {                                   \
       jthrow(jenv, "Invalid PDFlib object",                       \
           0, "pdflib_java.c");                                    \
       return _jresult;                                            \
    }
