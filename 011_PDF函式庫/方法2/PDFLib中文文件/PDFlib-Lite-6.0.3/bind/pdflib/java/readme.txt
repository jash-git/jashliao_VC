...
Problems under heavy load on Solaris systems
============================================

It seems that 32-bit versions of Java on Solaris sometimes are unable
to open enough file handles when a Web server is under heavy load.
As a result, PDFlib may be unable to load required resources, such
as the license file, or font, image, and input PDF files. We therefore
recommend the following:

- Switch to 64-bit versions of the Java runtime if possible.

- If you need to work with a 32-bit version of Java: set your PDFlib license
  key with a call to set_parameter(), or set the "nodemostamp" parameter to
  avoid the danger of inadvertently creating PDF output with the PDFlib demo
  stamp.
...
- pdf_java.dll or libpdf_java.so must be accessible in some system path,
  ...
  On Mac OS X copy pdflib_java.jnilib to Library/Java/Extensions.
...
- Put the PDFlib shared library or DLL (e.g. pdf_java.dll) into a directory
  which is accessible system-wide (e.g. C:\winnt\system32). On Mac OS X
  copy pdflib_java.jnilib to Library/Java/Extensions.
...
- On Unix systems, after building or installing PDFlib it may help to
  update the shared library cache via
