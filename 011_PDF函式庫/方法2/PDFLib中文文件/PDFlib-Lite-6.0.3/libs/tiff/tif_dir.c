/* PDFlib GmbH cvsid:
 * $Id: tif_dir.c,v 1.14.2.3 2005/08/08 20:14:03 tm Exp $ */
...
void _TIFFsetNString(TIFF* tif, char** cpp, char* cp, long n);

static void
setByteArray(TIFF* tif, void** vpp, void* vp, size_t nmemb, size_t elem_size)
{
	if (*vpp)
		_TIFFfree(tif, *vpp), *vpp = 0;
	if (vp) {
		tsize_t	bytes = nmemb * elem_size;
		if (elem_size && bytes / elem_size == nmemb)
			*vpp = (void*) _TIFFmalloc(tif, bytes);
		if (*vpp)
			_TIFFmemcpy(*vpp, vp, bytes);
	}
}
void _TIFFsetByteArray(TIFF* tif, void** vpp, void* vp, long n)
    { setByteArray(tif, vpp, vp, n, 1); }
void _TIFFsetString(TIFF* tif, char** cpp, char* cp)
    { setByteArray(tif, (void**) cpp, (void*) cp, strlen(cp)+1, 1); }
void _TIFFsetNString(TIFF* tif, char** cpp, char* cp, long n)
    { setByteArray(tif, (void**) cpp, (void*) cp, n, 1); }
void _TIFFsetShortArray(TIFF* tif, uint16** wpp, uint16* wp, long n)
    { setByteArray(tif, (void**) wpp, (void*) wp, n, sizeof (uint16)); }
void _TIFFsetLongArray(TIFF* tif, uint32** lpp, uint32* lp, long n)
    { setByteArray(tif, (void**) lpp, (void*) lp, n, sizeof (uint32)); }
void _TIFFsetFloatArray(TIFF* tif, float** fpp, float* fp, long n)
    { setByteArray(tif, (void**) fpp, (void*) fp, n, sizeof (float)); }
void _TIFFsetDoubleArray(TIFF* tif, double** dpp, double* dp, long n)
    { setByteArray(tif, (void**) dpp, (void*) dp, n, sizeof (double)); }

/*
 * Install extra samples information.
 */
static int
setExtraSamples(TIFF* tif, TIFFDirectory* td, va_list ap, int* v)
{
	uint16* va;
	int i;
	...
	*v = va_arg(ap, int);
	...
	_TIFFsetShortArray(tif, &td->td_sampleinfo, va, td->td_extrasamples);
	return (1);
}
...
_TIFFVSetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	case TIFFTAG_DOCUMENTNAME:
		_TIFFsetString(tif, &td->td_documentname, va_arg(ap, char*));
		break;
	case TIFFTAG_ARTIST:
		_TIFFsetString(tif, &td->td_artist, va_arg(ap, char*));
		break;
	case TIFFTAG_DATETIME:
		_TIFFsetString(tif, &td->td_datetime, va_arg(ap, char*));
		break;
	case TIFFTAG_HOSTCOMPUTER:
		_TIFFsetString(tif, &td->td_hostcomputer, va_arg(ap, char*));
		break;
	case TIFFTAG_IMAGEDESCRIPTION:
		_TIFFsetString(tif, &td->td_imagedescription,va_arg(ap, char*));
		break;
	case TIFFTAG_MAKE:
		_TIFFsetString(tif, &td->td_make, va_arg(ap, char*));
		break;
	case TIFFTAG_MODEL:
		_TIFFsetString(tif, &td->td_model, va_arg(ap, char*));
		break;
	case TIFFTAG_COPYRIGHT:
		_TIFFsetString(tif, &td->td_copyright, va_arg(ap, char*));
		break;
	case TIFFTAG_OPIIMAGEID:
		_TIFFsetString(tif, &td->td_opiimageid, va_arg(ap, char*));
		break;
	...
	case TIFFTAG_PAGENAME:
		_TIFFsetString(tif, &td->td_pagename, va_arg(ap, char*));
		break;
	...
	case TIFFTAG_COLORMAP:
		...
		_TIFFsetShortArray(tif, &td->td_colormap[0],
		    va_arg(ap, uint16*),v32);
		_TIFFsetShortArray(tif, &td->td_colormap[1],
		    va_arg(ap, uint16*),v32);
		_TIFFsetShortArray(tif, &td->td_colormap[2],
		    va_arg(ap, uint16*),v32);
		break;
	case TIFFTAG_EXTRASAMPLES:
		if (!setExtraSamples(tif, td, ap, &v))
			goto badvalue;
		break;
	case TIFFTAG_MATTEING:
		...
		if (td->td_extrasamples) {
			...
			_TIFFsetShortArray(tif, &td->td_sampleinfo, &sv, 1);
		}
		break;
	...
	/* Begin Pixar Tags */
 	...
 	case TIFFTAG_PIXAR_TEXTUREFORMAT:
 		_TIFFsetString(tif, &td->td_textureformat, va_arg(ap, char*));
 		break;
 	case TIFFTAG_PIXAR_WRAPMODES:
 		_TIFFsetString(tif, &td->td_wrapmodes, va_arg(ap, char*));
 		break;
 	case TIFFTAG_PIXAR_FOVCOT:
 		td->td_fovcot = (float) va_arg(ap, dblparam_t);
 		break;
 	case TIFFTAG_PIXAR_MATRIX_WORLDTOSCREEN:
 		_TIFFsetFloatArray(tif, &td->td_matrixWorldToScreen,
 			va_arg(ap, float*), 16);
 		break;
 	case TIFFTAG_PIXAR_MATRIX_WORLDTOCAMERA:
 		_TIFFsetFloatArray(tif, &td->td_matrixWorldToCamera,
 			va_arg(ap, float*), 16);
 		break;
 	/* End Pixar Tags */	       
	...
	case TIFFTAG_SUBIFD:
		if ((tif->tif_flags & TIFF_INSUBIFD) == 0) {
			...
			_TIFFsetLongArray(tif, &td->td_subifd,
			    va_arg(ap, uint32*), (long) td->td_nsubifd);
		} else {
	...
	case TIFFTAG_YCBCRCOEFFICIENTS:
		_TIFFsetFloatArray(tif, &td->td_ycbcrcoeffs,
			va_arg(ap, float*), 3);
		break;
	...
	case TIFFTAG_WHITEPOINT:
		_TIFFsetFloatArray(tif, &td->td_whitepoint,
			va_arg(ap, float*), 2);
		break;
	case TIFFTAG_PRIMARYCHROMATICITIES:
		_TIFFsetFloatArray(tif, &td->td_primarychromas,
			va_arg(ap, float*),6);
		break;
	case TIFFTAG_TRANSFERFUNCTION:
		v = (td->td_samplesperpixel - td->td_extrasamples) > 1 ? 3 : 1;
		for (i = 0; i < v; i++)
			_TIFFsetShortArray(tif, &td->td_transferfunction[i],
			    va_arg(ap, uint16*), 1L<<td->td_bitspersample);
		break;
	case TIFFTAG_REFERENCEBLACKWHITE:
		/* XXX should check for null range */
		_TIFFsetFloatArray(tif, &td->td_refblackwhite,
			va_arg(ap, float*),6);
		break;
	case TIFFTAG_INKSET:
	...
	case TIFFTAG_INKNAMES:
		...
		i = checkInkNamesString(tif, i, s);
                status = i > 0;
		if( i > 0 ) {
			_TIFFsetNString(tif, &td->td_inknames, s, i);
			td->td_inknameslen = i;
		}
		break;
	...
	case TIFFTAG_TARGETPRINTER:
		_TIFFsetString(tif, &td->td_targetprinter, va_arg(ap, char*));
		break;
	case TIFFTAG_ICCPROFILE:
		td->td_profileLength = (uint32) va_arg(ap, uint32);
		_TIFFsetByteArray(tif, &td->td_profileData, va_arg(ap, void*),
		    td->td_profileLength);
		break;
 	case TIFFTAG_PHOTOSHOP:
  		td->td_photoshopLength = (uint32) va_arg(ap, uint32);
  		_TIFFsetByteArray(tif, &td->td_photoshopData, va_arg(ap, void*),
 			td->td_photoshopLength);
 		break;
	case TIFFTAG_RICHTIFFIPTC: 
  		td->td_richtiffiptcLength = (uint32) va_arg(ap, uint32);
  		_TIFFsetLongArray (tif, (uint32**)&td->td_richtiffiptcData,
				   va_arg(ap, uint32*),
				   td->td_richtiffiptcLength);
 		break;
	case TIFFTAG_XMLPACKET:
		td->td_xmlpacketLength = (uint32) va_arg(ap, uint32);
		_TIFFsetByteArray(tif, &td->td_xmlpacketData, va_arg(ap, void*),
		...
		new_customValues = (TIFFTagValue *)
		    _TIFFrealloc(tif, td->td_customValues,
	    ...
	    /* PDFlib GmbH: inserted bug fix from libtiff 3.7.0 */
	    ...
	    switch (fip->field_type) {
            ...
            if( fip->field_passcount )
            {
                tv->value = _TIFFmalloc(tif, tv_size * tv->count);
		if ( !tv->value ) {
			va_end(ap);
			return 0;
		}
            ...
            else if( fip->field_type == TIFF_ASCII )
            {
                ...
                tv->value = _TIFFmalloc(tif, tv->count);
		if ( !tv->value ) {
			va_end(ap);
			return 0;
		}
...
_TIFFVGetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	case TIFFTAG_ICCPROFILE:
            ...
            *va_arg(ap, void**) = td->td_profileData;
            break;
	case TIFFTAG_OPIIMAGEID:
            *va_arg(ap, char**) = td->td_opiimageid;
            break;
 	case TIFFTAG_PHOTOSHOP:
        ...
        default:
        {
            ...
            */
            ...
            for( i = 0; i < td->td_customValueCount; i++ )
            {
                ...
                if( fip->field_passcount )
                {
                    *va_arg(ap, tif_short *) = (tif_short) tv->count;
...
TIFFFreeDirectory(TIFF* tif)
{
    ...
    for( i = 0; i < td->td_customValueCount; i++ )
        _TIFFfree(tif, td->td_customValues[i].value );

    if( td->td_customValues != NULL )
        _TIFFfree(tif, td->td_customValues );
