/* PDFlib GmbH cvsid
 * :$Id: tif_dirread.c,v 1.18.2.5 2005/08/09 21:48:32 tm Exp $
 */
...
TIFFReadDirectory(TIFF* tif)
{
	...
	tif->tif_dirnumber++;
	new_dirlist = (toff_t*) _TIFFrealloc(tif, tif->tif_dirlist,
	...
	if (!isMapped(tif)) {
		...
		/*
		* PDFlib GmbH: EFAX (*.jfx) files have a dircount of 0,
		* and cannot be processed by TIFFlib.
		*/
		if (dircount == 0)
		    return (0);
		 ...
		 */
		(void) ReadOK(tif, &nextdiroff, ((tsize_t)sizeof (uint32)));
	} else {
...
"%.1000s: invalid TIFF directory; tags are not sorted in ascending order",
	...
	ignore:
		...
		while (dp->tdir_type != (tif_short) fip->field_type) {
...
/* END REV 4.0 COMPATIBILITY */
	...
	if (!TIFFFieldSet(tif, FIELD_STRIPBYTECOUNTS)) {
		...
		TIFFWarning(module,
			...
			tif->tif_name, TIFFFieldWithTag(tif,
...
#if 0
	/* oversize.tif can no longer be processed with the following checks */
	if (!tif->tif_tilesize) {
...
EstimateStripByteCounts(TIFF* tif, TIFFDirEntry* dir, uint16 dircount)
{
	...
	if (td->td_stripbytecount)
		_TIFFfree(tif, td->td_stripbytecount);
...
cvtRational(TIFF* tif, TIFFDirEntry* dir, uint32 num, uint32 denom, float* rv)
{
	(void) tif;
        ...
        if (denom == 0) {
            /* PDFlib GmbH
             * Some files have damaged rationals with denom = 0 when
             * they actually mean denom = 1!
             */
            denom = 1;
        }
...
/*
 * PDFlib GmbH:
 * Disabled reading of tags with type ASCII and undefined; we
 * never use these, and some malformed TIFFs can cause problems
 */
#if 0
			{ char c[2];
...
ChopUpSingleUncompressedStrip(TIFF* tif)
{
	...
	if (newcounts == NULL || newoffsets == NULL) {
		 ...
		 */
		if (newcounts != NULL)
			_TIFFfree(tif, newcounts);
		if (newoffsets != NULL)
			_TIFFfree(tif, newoffsets);
		return;
	}
	/*
	...
	_TIFFfree(tif, td->td_stripbytecount);
	_TIFFfree(tif, td->td_stripoffset);
