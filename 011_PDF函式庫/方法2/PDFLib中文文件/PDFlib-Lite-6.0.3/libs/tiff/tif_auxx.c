/* PDFlib GmbH cvsid: $Id: tif_auxx.c,v 1.4.2.1 2004/11/10 15:55:13 tm Exp $ */
...
TIFFDefaultTransferFunction(TIFF* tif, TIFFDirectory* td)
{
	...
	tsize_t i, n, nbytes;
 	...
 	if (!(tf[0] = (uint16 *)_TIFFmalloc(tif, nbytes)))
 		return 0;
	tf[0][0] = 0;
	for (i = 1; i < n; i++) {
	...
	if (td->td_samplesperpixel - td->td_extrasamples > 1) {
 		if (!(tf[1] = (uint16 *)_TIFFmalloc(tif, nbytes)))
 			goto bad;
 		_TIFFmemcpy(tf[1], tf[0], nbytes);
 		if (!(tf[2] = (uint16 *)_TIFFmalloc(tif, nbytes)))
 			goto bad;
...
TIFFDefaultRefBlackWhite(TIFF* tif, TIFFDirectory* td)
{
	...
	td->td_refblackwhite = (float *)_TIFFmalloc(tif, 6*sizeof (float));
	if(!(td->td_refblackwhite = (float *)_TIFFmalloc(tif,6*sizeof (float))))
		return 0;
	for (i = 0; i < 3; i++) {
	...
	case TIFFTAG_YCBCRCOEFFICIENTS:
		if (!td->td_ycbcrcoeffs) {
			td->td_ycbcrcoeffs = (float *)
			    _TIFFmalloc(tif, 3*sizeof (float));
 			if (!td->td_ycbcrcoeffs)
 				return (0);
	...
	case TIFFTAG_WHITEPOINT:
		if (!td->td_whitepoint) {
			td->td_whitepoint = (float *)
				_TIFFmalloc(tif, 2 * sizeof (float));
 			if (!td->td_whitepoint)
 				return (0);
