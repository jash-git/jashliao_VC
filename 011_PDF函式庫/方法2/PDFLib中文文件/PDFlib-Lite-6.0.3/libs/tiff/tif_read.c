/* PDFlib GmbH cvsid:
 * $Id: tif_read.c,v 1.10 2004/03/26 18:36:54 rjs Exp $ */
...
TIFFSeek(TIFF* tif, uint32 row, tsample_t sample)
{
	...
	if (row >= td->td_imagelength) {	/* out of range */
		TIFFError(tif->tif_name, "%lu: Row out of range, max %lu",
		    (tif_long) row, (tif_long) td->td_imagelength);
		return (0);
	}
	if (td->td_planarconfig == PLANARCONFIG_SEPARATE) {
		if (sample >= td->td_samplesperpixel) {
			TIFFError(tif->tif_name,
			    ...
			    (tif_long)sample, (tif_long)td->td_samplesperpixel);
			return (0);
		}
    ...
    tstrip_t strip, tdata_t buf, tsize_t size, const char* module)
{
	...
	if (!isMapped(tif)) {
		...
		if (!SeekOK(tif, td->td_stripoffset[strip])) {
			TIFFError(module,
			    ...
			    tif->tif_name,
			    (tif_long) tif->tif_row, (tif_long) strip);
			return (-1);
		}
    ...
    "%s: Read error at scanline %lu, strip %lu; got %lu bytes, expected %lu",
			    ...
			    (tif_long) tif->tif_row,
			    (tif_long) strip,
			    (tif_long)tif->tif_size - td->td_stripoffset[strip],
			    (tif_long) size);
			return (-1);
		}
...
TIFFReadRawStrip(TIFF* tif, tstrip_t strip, tdata_t buf, tsize_t size)
{
	...
	if (strip >= td->td_nstrips) {
		TIFFError(tif->tif_name, "%lu: Strip out of range, max %lu",
		    (tif_long) strip, (tif_long) td->td_nstrips);
		return ((tsize_t) -1);
	}
...
TIFFFillStrip(TIFF* tif, tstrip_t strip)
{
	...
	if (isMapped(tif) &&
		...
		if (TIFFReadRawStrip1(tif, strip, (tif_char *)tif->tif_rawdata,
...
TIFFReadEncodedTile(TIFF* tif, ttile_t tile, tdata_t buf, tsize_t size)
{
	...
	if (tile >= td->td_nstrips) {
		TIFFError(tif->tif_name, "%ld: Tile out of range, max %ld",
		    (long) tile, (tif_long) td->td_nstrips);
		return (-1);
	}
	if (size == (tsize_t) -1)
		size = tilesize;
    ...
    ttile_t tile, tdata_t buf, tsize_t size, const char* module)
{
    ...
    "%s: Read error at row %ld, col %ld, tile %ld; got %lu bytes, expected %lu",
			    ...
			    (long) tif->tif_col,
			    (long) tile,
			    (tif_long) tif->tif_size - td->td_stripoffset[tile],
			    (tif_long) size);
			return ((tsize_t) -1);
		}
...
TIFFReadRawTile(TIFF* tif, ttile_t tile, tdata_t buf, tsize_t size)
{
	...
	if (tile >= td->td_nstrips) {
		TIFFError(tif->tif_name, "%lu: Tile out of range, max %lu",
		    (tif_long) tile, (tif_long) td->td_nstrips);
		return ((tsize_t) -1);
	}
...
TIFFFillTile(TIFF* tif, ttile_t tile)
{
	...
	if (bytecount <= 0) {
		TIFFError(tif->tif_name,
		    ...
		    (tif_long) bytecount, (tif_long) tile);
		return (0);
	}
	if (isMapped(tif) &&
	    (isFillOrder(tif, td->td_fillorder) ||
		...
		if (TIFFReadRawTile1(tif, tile, (tif_char *)tif->tif_rawdata,
		    bytecount, module) != bytecount)
			return (0);
		if (!isFillOrder(tif, td->td_fillorder) &&
		    (tif->tif_flags & TIFF_NOBITREV) == 0)
...
TIFFReadBufferSetup(TIFF* tif, tdata_t bp, tsize_t size)
{
	...
	if (bp) {
		...
		tif->tif_rawdata =
		    (tidata_t) _TIFFmalloc(tif, tif->tif_rawdatasize);
		tif->tif_flags |= TIFF_MYBUFFER;
	}
