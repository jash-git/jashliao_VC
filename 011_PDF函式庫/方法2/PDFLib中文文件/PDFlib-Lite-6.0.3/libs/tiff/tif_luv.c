/* PDFlib GmbH cvsid:
 * $Id: tif_luv.c,v 1.5.2.2 2004/11/10 15:55:13 tm Exp $ */
/*
			...
			while (i < beg) {		/* write out non-run */
				...
				if (occ < j+3) {
					...
					if (!TIFFFlushData1(tif))
						return (-1);
					op = tif->tif_rawcp;
					occ = tif->tif_rawdatasize
					    - tif->tif_rawcc;
				}
				*op++ = (tidataval_t) j; occ--;
				while (j--) {
					*op++ = (tidataval_t)(tp[i++] >> shft
					    & 0xff);
					occ--;
				}
			}
...
#define	PACK(s,b,f)	(((b)<<6)|((s)<<3)|(f))
	switch (PACK(td->td_samplesperpixel, td->td_bitspersample,
			    td->td_sampleformat)) {
...
LogL16InitState(TIFF* tif)
{
	...
	if (multiply(sp->tbuflen, sizeof (int16)) == 0 ||
	    (sp->tbuf = (tidata_t*)_TIFFmalloc(tif,sp->tbuflen * sizeof(int16)))
		== NULL) {
...
LogLuvInitState(TIFF* tif)
{
	...
	if (multiply(sp->tbuflen, sizeof (uint32)) == 0 ||
	    (sp->tbuf = (tidata_t*)_TIFFmalloc(tif, sp->tbuflen*sizeof(uint32)))
		    == NULL) {
		TIFFError(module, "%s: No space for SGILog translation buffer",
		    tif->tif_name);
		return (0);
...
TIFFInitSGILog(TIFF* tif, int scheme)
{
	 ...
	 */
	tif->tif_data = (tidata_t) _TIFFmalloc(tif, sizeof (LogLuvState));
	...
	TIFFMergeFieldInfo(tif, LogLuvFieldInfo, N(LogLuvFieldInfo));
