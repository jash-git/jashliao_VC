/* PDFlib GmbH cvsid: $Id: tif_next.c,v 1.7.2.1 2004/11/09 09:56:11 tm Exp $ */
...
#define SETPIXEL(op, v) {			\
	switch (npixels++ & 3) {		\
	case 0:	op[0]  = (tif_char) ((v) << 6); break;	\
...
NeXTDecode(TIFF* tif, tidata_t buf, tsize_t occ, tsample_t s)
{
	...
	for (row = buf; (long)occ > 0; occ -= scanline, row += scanline) {
		...
		default: {
			...
			tif_long imagewidth = tif->tif_dir.td_imagewidth;
