/* PDFlib GmbH cvsid:
 * $Id: tif_jpeg.c,v 1.8.2.4 2004/09/29 14:37:13 tm Exp $
 */
...
tables_empty_output_buffer(/*TIFF* tif,*/ j_compress_ptr cinfo)
{
	...
	newbuf = _TIFFrealloc(tif, (tdata_t) sp->jpegtables,
...
TIFFjpeg_tables_dest(JPEGState* sp, TIFF* tif)
{
	 ...
	 */
	if (sp->jpegtables)
		_TIFFfree(tif, sp->jpegtables);
	sp->jpegtables_length = 1000;
	sp->jpegtables=(void*)_TIFFmalloc(tif, (tsize_t)sp->jpegtables_length);
...
JPEGCleanup(TIFF* tif)
{
	if (tif->tif_data) {
		...
		if (sp->jpegtables)		/* tag value */
			_TIFFfree(tif, sp->jpegtables);
		_TIFFfree(tif, tif->tif_data);	/* release local state */
		tif->tif_data = NULL;
	}
}
...
JPEGVGetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	switch (tag) {
	case TIFFTAG_JPEGTABLES:
		/* tif_short is bogus --- should be uint32 ??? */
		/* TIFFWriteNormalTag needs fixed  XXX */
		*va_arg(ap, tif_short*) = (tif_short) sp->jpegtables_length;
...
TIFFInitJPEG(TIFF* tif, int scheme)
{
	 ...
	 */
	tif->tif_data = (tidata_t) _TIFFmalloc(tif, sizeof (JPEGState));
