/* PDFlib GmbH cvsid:
 * $Id: tif_getimage.c,v 1.15.2.1 2004/11/10 15:55:13 tm Exp $ */
...
static	int pickTileContigCase(TIFF*, TIFFRGBAImage*);
static	int pickTileSeparateCase(TIFF*, TIFFRGBAImage*);
...
void
TIFFRGBAImageEnd(TIFF* tif, TIFFRGBAImage* img)
{
	if (img->Map)
	...
	if (img->cielab)
		_TIFFfree(tif, img->cielab), img->cielab = NULL;
...
TIFFRGBAImageBegin(TIFFRGBAImage* img, TIFF* tif, int stop, char emsg[1024])
{
	...
	return pickTileContigCase(tif, img);
    } else {
	img->get = TIFFIsTiled(tif) ? gtTileSeparate : gtStripSeparate;
	return pickTileSeparateCase(tif, img);
    }
}
...
gtTileContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    buf = (tif_char*) _TIFFmalloc(tif, TIFFTileSize(tif));
    if (buf == 0) {
...
gtTileSeparate(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    buf = (tif_char*) _TIFFmalloc(tif, 4*tilesize);
    if (buf == 0) {
	TIFFError(TIFFFileName(tif), "No space for tile buffer");
	return (0);
    }
...
gtStripContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    buf = (tif_char*) _TIFFmalloc(tif, TIFFStripSize(tif));
    if (buf == 0) {
...
gtStripSeparate(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    r = buf = (tif_char *)_TIFFmalloc(tif, 4*stripsize);
    if (buf == 0) {
	TIFFError(TIFFFileName(tif), "No space for tile buffer");
	return (0);
    }
...
static void name(\
    ...
    uint32 w, uint32 h,\
    int32 fromskew, int32 toskew,\
    tif_char* r, tif_char* g, tif_char* b, tif_char* a\
)
...
initYCbCrConversion(TIFFRGBAImage* img)
{
	...
	if (img->ycbcr == NULL) {
	    img->ycbcr = (TIFFYCbCrToRGB*) _TIFFmalloc(img->tif,
...
initCIELabConversion(TIFFRGBAImage* img)
{
	...
	if (!img->cielab) {
		img->cielab = (TIFFCIELabToRGB *)
			_TIFFmalloc(img->tif, sizeof(TIFFCIELabToRGB));
		if (!img->cielab) {
			TIFFError(module,
 ...
 */
static int
makebwmap(TIFF* tif, TIFFRGBAImage* img)
{
    TIFFRGBValue* Map = img->Map;
 ...
 */
static int
setupMap(TIFF* tif, TIFFRGBAImage* img)
{
    int32 x, range;
    ...
    img->Map = (TIFFRGBValue*) _TIFFmalloc(tif, (range+1)*sizeof(TIFFRGBValue));
    if (img->Map == NULL) {
	TIFFError(TIFFFileName(img->tif),
 ...
 */
static int
makecmap(TIFF* tif, TIFFRGBAImage* img)
{
    int bitspersample = img->bitspersample;
    int nsamples = 8 / bitspersample;
    uint16* r = img->redcmap;
 ...
 */
static int
buildMap(TIFF* tif, TIFFRGBAImage* img)
{
    switch (img->photometric) {
    case PHOTOMETRIC_RGB:
    case PHOTOMETRIC_YCBCR:
	 ...
	 * unpacking tables for samples < 8 bits.
	 */
	if (img->bitspersample <= 8 && !makecmap(tif, img))
	    return (0);
	break;
    }
    return (1);
 ...
 */
static int
pickTileContigCase(TIFF* tif, TIFFRGBAImage* img)
{
 ...
 */
static int
pickTileSeparateCase(TIFF* tif, TIFFRGBAImage* img)
{
