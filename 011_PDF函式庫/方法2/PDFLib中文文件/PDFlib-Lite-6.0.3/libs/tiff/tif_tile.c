/* PDFlib GmbH cvsid: $Id: tif_tile.c,v 1.7.2.1 2004/11/10 15:55:13 tm Exp $ */
...
multiply(TIFF* tif, size_t nmemb, size_t elem_size, const char* where)
{
	...
	/* tm Hack: the tile size may be -1, and the overflow check could
	 * bail out even though the estimation logic works fine.
	 */
	if (nmemb == (uint32) -1)
	    return (uint32) -1;
...
TIFFVTileSize(TIFF* tif, uint32 nrows)
{
	...
	if (td->td_planarconfig == PLANARCONFIG_CONTIG &&
		...
		tilesize = multiply(tif, nrows, TIFFTileRowSize(tif),
				    "TIFFVTileSize");
	if (nrows == -1)
	    return ((tsize_t)(nrows * TIFFTileRowSize(tif) * td->td_tiledepth));
	else
	    return ((tsize_t)
