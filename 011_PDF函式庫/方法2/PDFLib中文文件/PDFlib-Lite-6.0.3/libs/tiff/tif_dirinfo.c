/* PDFlib GmbH cvsid:
 * $Id: tif_dirinfo.c,v 1.13.2.3 2005/08/09 21:48:32 tm Exp $ */
...
/* XXX temporarily accept LONG for backwards compatibility */
    ...
    { TIFFTAG_OPIIMAGEID,		-1,-1, TIFF_ASCII, FIELD_OPIIMAGEID,
      TRUE,	FALSE,	"OPIImageID" },
/* begin SGI tags */
...
TIFFMergeFieldInfo(TIFF* tif, const TIFFFieldInfo info[], int n)
{
	...
	if (tif->tif_nfields > 0) {
		tif->tif_fieldinfo = (TIFFFieldInfo**)
		    _TIFFrealloc(tif, tif->tif_fieldinfo,
			(tif->tif_nfields+n) * sizeof (TIFFFieldInfo*));
	} else {
		tif->tif_fieldinfo = (TIFFFieldInfo**)
		    _TIFFmalloc(tif, n * sizeof (TIFFFieldInfo*));
	}
...
/* PDFlib GmbH: inserted new function implementation for bugfix from
 * libtiff 3.7.0
 */

const TIFFFieldInfo*
TIFFFindFieldInfo(TIFF* tif, ttag_t tag, TIFFDataType dt)
{
	int i, n;
...
TIFFFieldWithTag(TIFF* tif, ttag_t tag)
{
	...
	if (!fip) {
		TIFFError("TIFFFieldWithTag",
		    "Internal error, unknown tag 0x%x", (tif_int) tag);
		/*NOTREACHED*/
	}
	return (fip);
}
...
_TIFFCreateAnonFieldInfo(TIFF *tif, ttag_t tag, TIFFDataType field_type)
{
    ...
    fld = (TIFFFieldInfo *) _TIFFmalloc(tif, sizeof (TIFFFieldInfo));
    ...
    fld->field_name = (char *) _TIFFmalloc(tif, 32);
