/* PDFlib GmbH cvsid:
 * $Id: tif_ojpeg.c,v 1.13.2.6 2006/01/29 23:26:05 tm Exp $ */
#include "tiffiop.h"
#ifdef OJPEG_SUPPORT
...
#   endif
    ...
    tif_char h_sampling,                        /* Luminance sampling factors */
           v_sampling,
...
#   define sp ((OJPEGState *)cinfo)
 ...
 /* The entire buffer has been filled, so enlarge it by 1000 bytes. */
    if (!( newbuf = _TIFFrealloc(tif,  (tdata_t)sp->jpegtables
 ...
 */
    if (sp->jpegtables) _TIFFfree(tif, sp->jpegtables);
    if (!(sp->jpegtables = (void*)
                     _TIFFmalloc(tif, (tsize_t)(sp->jpegtables_length = 1000))
         )
       )
      {
       ...
       */
          if (   (   td->td_ycbcrsubsampling[0] == 1
            ...
            sp->cinfo.c.raw_data_in =
              ( (sp->h_sampling = (tif_char)(td->td_ycbcrsubsampling[0])) << 3
              | (sp->v_sampling = (tif_char)(td->td_ycbcrsubsampling[1]))
              ) != 011;
          else
            {
     ...
     */
     ...
     L: if (p - (unsigned char *)sp->jpegtables > 2) /* fake "JPEGTables" */
         ...
         */
            ...
            if (!(sp->jpegtables = _TIFFmalloc(tif, sp->jpegtables_length)))
              {
...
#   define td (&tif->tif_dir)
     ...
     */
        case TIFFTAG_REFERENCEBLACKWHITE   : if (td->td_refblackwhite)
                                               {
                                          _TIFFfree(tif, td->td_refblackwhite);
     ...
     */
             ...
             )
	  {
            if ( (td->td_refblackwhite =
                  (float *) _TIFFmalloc(tif, 6*sizeof(float))) )
...
#             endif /* C_LOSSLESS_SUPPORTED */
     ...
     */
        case TIFFTAG_JPEGLOSSLESSPREDICTORS:
           if (v32)
		    ...
		    _TIFFmalloc(tif, sp->jpeglosslesspredictors_length
				* sizeof(uint16));
     ...
     */
		...
		_TIFFmalloc(tif, sp->jpegpointtransform_length*sizeof(uint16));
        ...
        case TIFFTAG_JPEGQTABLES           :
          if (v32)
            { uint32 *v;
              ...
              i = 0;
              v = va_arg(ap,uint32 *);
                sp->jpegqtables=_TIFFmalloc(tif, 64*sp->jpegqtables_length);
                ...
                for(i2=0;(uint16)i2<sp->jpegqtables_length;i2++){
                    ...
                    TIFFReadFile(tif, &(((tif_char*)(sp->jpegqtables))[bufoff]),
				 64);
                    bufoff+=64;
                }
     ...
     /* We have a vector of offsets to AC Huffman tables, so load 'em! */
       ...
       L: if (v32)
		...
		sp->jpegdctables=_TIFFmalloc(tif, 272*sp->jpegdctables_length);
                    ...
                    for(i2=0;(uint16)i2<sp->jpegdctables_length;i2++){
                        TIFFSeekFile(tif, v[i2], SEEK_SET);
                        TIFFReadFile(tif,
				     &(((tif_char*)(sp->jpegdctables))[bufoff]),
				     16);
                        code_count=0;
                        for(k2=0;k2<16;k2++){
			code_count+=((tif_char*)(sp->jpegdctables))[k2+bufoff];
                        }
			...
			if (code_count > 256)
			{
			    TIFFError(JPEGLib_name, "DC Huffman table corrupt");
			    return 0;
			}
                        TIFFReadFile(tif,
				 &(((tif_char*)(sp->jpegdctables))[bufoff+16]),
				     code_count);
                        bufoff+=16;
                        bufoff+=code_count;
                    }
		...
		sp->jpegactables=_TIFFmalloc(tif, 272*sp->jpegactables_length);
			...
			if (code_count > 256)
			{
			    TIFFError(JPEGLib_name, "AC Huffman table corrupt");
			    return 0;
			}
       ...
       */
       ...
       /* If the up-sampling state changed, re-calculate tile size. */
          ...
          if ((tif->tif_flags ^ v32) & TIFF_UPSAMPLED)
            {
	      tif->tif_tilesize = isTiled(tif) ? TIFFTileSize(tif) : (tsize_t)
				   -1;
              tif->tif_flags |= TIFF_DIRTYDIRECT;
            };
          return 1;
  ...
  { register OJPEGState *sp;
    ...
    if ( (sp = OJState(tif)) )
      {
        ...
        if (sp->jpegtables) {_TIFFfree(tif, sp->jpegtables);sp->jpegtables=0;}
        if (sp->jpeglosslesspredictors) {
		_TIFFfree(tif, sp->jpeglosslesspredictors);
		sp->jpeglosslesspredictors = 0;
	}
        if (sp->jpegpointtransform) {
		_TIFFfree(tif, sp->jpegpointtransform);
		sp->jpegpointtransform=0;
	}
        if (sp->jpegqtables) {_TIFFfree(tif,sp->jpegqtables);sp->jpegqtables=0;}
        if (sp->jpegactables) {
	    _TIFFfree(tif, sp->jpegactables);sp->jpegactables=0;
	}
        if (sp->jpegdctables) {
	    _TIFFfree(tif, sp->jpegdctables);sp->jpegdctables=0;
	}
     ...
     */
        if (!isMapped(tif) && tif->tif_base) /* free whole-file buffer */
          {
            _TIFFfree(tif, tif->tif_base);
            tif->tif_base = 0;
            tif->tif_size = 0;
          };
        _TIFFfree(tif, sp); /* Release local variables */
        tif->tif_data = 0;
      }
  }
...
#   endif /* never */
 ...
 /* Allocate storage for this module's per-file variables. */
    ...
    if (!(tif->tif_data = (tidata_t)_TIFFmalloc(tif, sizeof *sp)))
      {
