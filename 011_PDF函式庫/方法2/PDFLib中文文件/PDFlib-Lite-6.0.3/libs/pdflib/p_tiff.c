...
/* $Id: p_tiff.c,v 1.101.2.11 2006/02/15 12:51:20 tm Exp $
 *
...
pdf_data_source_TIFF_init(PDF *p, PDF_data_source *src)
{
	...
	if (image->bpc == 1)
	    src->buffer_length =
		(size_t) (image->components * (image->width+7)/8);
	else
	    src->buffer_length =
...
pdf_is_TIFF_file(PDF *p, pdc_file *fp, pdf_tiff_info *tiff_info, pdc_bool check)
{
    ...
    pdc_trace_protocol(p->pdc, 1, trc_image, "\tChecking image type TIFF...\n");
...
#define PDF_REALLY_BIG_DPI	10000
	...
	case COMPRESSION_OJPEG:
		...
		jpegifoffset != 0 &&
	       !TIFFGetField(MYTIFF, TIFFTAG_JPEGIFBYTECOUNT, &jpegifbytecount))
	    {
		/* stop TIFF processing */
		TIFFClose(MYTIFF);

		/* store data offset for the JPEG module (after TIFFClose()
		 * the image->info union is no longer used by the TIFF module)
		 */
	...
	if (image->components == 1 && image->bpc == 1)
	{
	     ...
	     * and therefore don't change bpc.
	     */
	    ...
	    {
		/* Apparently libtiff ignores photometric when retrieving
		 * fax data pixel by pixel.
		 */
		image->invert = !image->invert;
	    }
	}
