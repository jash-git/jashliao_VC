...
/* $Id: p_jpeg.c,v 1.70.2.38 2006/02/24 11:37:29 kurt Exp $
 *
...
pdf_output_message_src(j_common_ptr cinfo)
{
    ...
    pdc_trace(src->p->pdc, "\tlibjpeg src: %s\n", buffer);
}
...
pdf_output_message_dst(j_common_ptr cinfo)
{
    ...
    if (!pdc_trace_protocol_is_enabled(dst->p->pdc, 5, trc_image))
	return;

    /* Create the message */
    (*cinfo->err->format_message) (cinfo, buffer);

    pdc_trace(dst->p->pdc, "\tlibjpeg dst: %s\n", buffer);
}
...
pdf_error_exit_src(j_common_ptr cinfo)
{
	...
	pdc_trace(p->pdc, "\tlibjpeg (src) called error_exit routine\n");
...
pdf_error_exit_dst(j_common_ptr cinfo)
{
    ...
    if (pdc_trace_protocol_is_enabled(p->pdc, 5, trc_image))
	pdc_trace(p->pdc, "\tlibjpeg (dst) called error_exit routine\n");
...
pdf_data_source_JPEG_fill_transcode(PDF *p, PDF_data_source *src)
{
    ...
    {
	if (pdc_trace_protocol_is_enabled(p->pdc, 5, trc_image))
	    pdc_trace(p->pdc, "\tlibjpeg couldn't read header\n");
    ...
    {
	if (pdc_trace_protocol_is_enabled(p->pdc, 5, trc_image))
	    pdc_trace(p->pdc, "\tlibjpeg couldn't read coefficients\n");
	...
	pdc_trace_protocol_is_enabled(p->pdc, 5, trc_image))
    {
	/*
...
pdf_is_JPEG_file(PDF *p, pdc_file *fp)
{
    ...
    pdc_trace_protocol(p->pdc, 1, trc_image, "\tChecking image type JPEG...\n");
...
pdf_log_colorspace(PDF *p, int slot)
{
    ...
    {
        pdc_trace(p->pdc, " Bad color space slot %d", slot);
    }
    ...
    switch (cs->type) {
        case DeviceGray:
        pdc_trace(p->pdc, "/DeviceGray");
        break;

        case DeviceRGB:
        pdc_trace(p->pdc, "/DeviceRGB");
        break;

        case DeviceCMYK:
        pdc_trace(p->pdc, "/DeviceCMYK");
        break;


        case Indexed:
        pdc_trace(p->pdc, "/Indexed");
        break;

        case PatternCS:
        pdc_trace(p->pdc, "/Pattern");
        break;

        default:
        pdc_trace(p->pdc, "%d (unknown)", cs->type);
    }
}
...
#define APP_MAX 255
	...
	if (need_transcode)
	    pdc_trace(p->pdc, "\ttranscoding...\n");
	else
	    pdc_trace(p->pdc, "\ttranscoding disabled by passthrough option\n");
    }
    ...
    while (1)
    {
        ...
        if (!markers_done)
        {
            ...
            {
                if (traceprotocol && pos > (long) image->info.jpeg.jpegifoffset)
                {
...
#define PDF_STRING_Adobe   "\x41\x64\x6F\x62\x65"
	...
	if (traceprotocol)
	{
	    pdc_trace(p->pdc, "\tcalling libjpeg for transcoding\n");
	}
	image->src.init		= NULL;
