...
/* $Id: p_truetype.c,v 1.220.2.59 2005/11/14 11:21:58 kurt Exp $
 *
...
tt_unicode2gidx4(PDF *p, tt_file *ttf, unsigned int uv)
{
        ...
        if (idx < 0 || idx >= cm4->numGlyphIds)
        {
            if (ttf->font->verbose)
                pdc_warning(p->pdc, PDF_E_TT_GLYPHIDNOTFOUND,
                            ...
                            pdc_errprintf(p->pdc, "%s", ttf->fontname), 0, 0);
            return 0;
        }
    ...
    gidx = (gidx < ttf->numGlyphs) ? gidx : 0;
    return gidx;
...
tt_code2gidx(PDF *p, tt_file *ttf, pdc_font *font, int code)
{
    ...
    else if (font->isstdlatin == pdc_false)
    {
        ...
        if (!ttf->tab_OS_2)
        {
            pdc_set_errmsg(p->pdc, PDF_E_TT_SYMBOLOS2, ttf->fontname, 0, 0, 0);
	    return -1;
        }
        ...
        */
        ...
        if (enc != pdc_builtin && code >= uvoffset)
            uv = (pdc_ushort) code;
        else
...
tt_get_tab_hhea(PDF *p, tt_file *ttf)
{
    ...
    tp->advanceWidthMax		= tt_get_fword(p, ttf);
...
tt_get_tab_hmtx(PDF *p, tt_file *ttf)
{
    ...
    */
    ...
    {
	tp->metrics[i].advanceWidth = tt_get_fword(p, ttf);
...
tt_get_tab_CFF_(PDF *p, tt_file *ttf)
{
    ...
    else
    {
        ...
        {
            pdc_set_errmsg(p->pdc, PDF_E_TT_NOGLYFDESC, ttf->fontname, 0, 0, 0);
	    return pdc_false;
        }
        idx = -1;
    }
...
tt_get_tab_name(PDF *p, tt_file *ttf)
{
    ...
    */
    ...
    {
        pdc_set_errmsg(p->pdc, PDF_E_TT_NONAME, ttf->fontname, 0, 0, 0);
        return pdc_false;
    }
...
tt_get_tab_OS_2(PDF *p, tt_file *ttf)
{
    ...
    {
        tp->sxHeight = PDC_MISSING_FONTVAL;
        tp->sCapHeight = PDC_MISSING_FONTVAL;
	tp->usDefaultChar = 0;
	tp->usBreakChar = 0;
	tp->usMaxContext = 0;
    }
...
tt_get_tab_post(PDF *p, tt_file *ttf)
{
    ...
    tp->italicAngle = (tt_get_fixed(p, ttf) / 65536.0);
...
static const char *pdf_filetypes[4] =
{
    "TrueType", "OpenType", "Apple TrueType", "TrueType Collection"
};

pdc_bool
pdf_test_tt_font(PDF *p, tt_byte *img, tt_ulong *n_fonts)
{
    tt_ushort n_tables;
    int ift = 0;
...
pdf_read_offset_tab(PDF *p, tt_file *ttf)
{
    ...
    if (pdf_test_tt_font(p, img, NULL) == pdc_false)
    {
	pdc_set_errmsg(p->pdc, PDF_E_TT_NOFONT, ttf->filename, 0, 0, 0);
	return pdc_false;
    }
    ...
    {
	pdc_set_errmsg(p->pdc, PDF_E_TT_BITMAP, ttf->fontname, 0, 0, 0);
	return pdc_false;
    }
...
pdf_set_tt_fontvalues(PDF *p, tt_file *ttf, pdc_font *font)
{
    ...
    font->italicAngle        = ttf->tab_post->italicAngle;
    ...
    if (ttf->tab_OS_2)
    {
        double w        = ttf->tab_OS_2->usWeightClass / 65.0;
                   ...
                   pdc_encoding enc, const char *filename)
{
    ...
    const char *stemp1 = fontname, *stemp2 = NULL;
    ...
    if (enc == pdc_cid)
    {
        if (font->charcoll != ttf->charcoll)
        {
            stemp2 = font->cmapname;
            errcode = PDF_E_CJK_UNSUPP_CHARCOLL;
            goto PDF_TRUETYPE_ERROR1;
        }
    }
    else if (ttf->charcoll != cc_none && enc >= pdc_builtin)
    {
        stemp2 = font->encapiname;
        ...
        if (enc >= pdc_builtin && enc != pdc_macroman)
        {
            if (font->verbose && strcmp(font->encapiname, "auto"))
            {
                pdc_warning(p->pdc, PDF_E_FONT_FORCEENC,
                            pdf_get_encoding_name(p, pdc_macroman, NULL),
                            pdf_get_encoding_name(p, enc, font), fontname, 0);
            }
            enc = pdc_macroman;
        }
        pdf_get_encoding_vector(p, pdc_macroman);
    }
    ...
    {
        stemp2 = pdf_get_encoding_name(p, enc, font);
...
#define PDF_RESTRICTED_TT_EMBEDDING	0x02
    ...
    pdc_trace_protocol(p->pdc, 1, trc_font,
        ...
        "\tFont embedding: %s\n"
        "\tNumber of Glyphs: %d\n",
        font->name, font->ttname, PDC_BOOLSTR(font->embedding),ttf->numGlyphs);
    ...
    font->numOfGlyphs = ttf->numGlyphs;
    ...
    for (code = 0; code < ncodes; code++)
    {
        ...
        if (gidx)
        {
             ...
             * We prefer the first occurence (SPACE) (see above)
             */
    ...
    if (pdc_trace_protocol_is_enabled(p->pdc, 2, trc_font))
    {
        ...
        for (gidx = 0; gidx < font->numOfGlyphs; gidx++)
        {
            code = (int) ttf->GID2Code[gidx];
            width = tt_gidx2width(p, ttf, gidx);

            {
                if (enc >= 0)
                    glyphname = ev->chars[code];
                else if (enc != pdc_builtin)
                    glyphname = (char *) pdc_unicode2adobe((pdc_ushort) code);
            }
            if (glyphname == NULL)
                glyphname = "";
            ...
            switch (enc)
            {
                ...
                if (!gidx || code)
                {
                    ...
                    {
                        pdc_trace(p->pdc,
                            "\t\tGlyph%5d: Code=%3d  U+%04X  "
                            "Width=%4d  \"%s\"\n",
                            gidx, code, ev->codes[code], width, glyphname);
                    }
                    else
                    {
                        pdc_trace(p->pdc,
                            "\t\tGlyph%5d: Code=%3d  Width=%4d  \"%s\"\n",
                            gidx, code, width, glyphname);
                    }
                }
                break;
            }
    ...
    /* default width */
    font->defWidth = tt_gidx2width(p, ttf, 0);
    ...
    PDF_TRUETYPE_ERROR1:
    pdc_set_errmsg(p->pdc, errcode, stemp1, stemp2, 0, 0);
                       ...
                       pdc_font *font, tt_ulong n_fonts)
{
    ...
    if (ift == -1)
    {
        ...
        if (pdc_convert_string(p->pdc, textformat, 0, NULL,
    ...
    for (i = 0; i < (int)n_fonts; ++i)
    {
        ...
        pdc_trace_protocol(p->pdc, 1, trc_font, "\tChecking font #%d \n", i+1);
                  ...
                  pdc_font *font)
{
    ...
    fp = pdf_fopen_name(p, filename, fullname, "font ", PDC_FILE_BINARY);
    if (fp == NULL)
    {
    ...
    else
    {
        ...
        if (PDC_OK_FREAD(fp, img, TT_OFFSETTAB_SIZE))
        {
            ...
            retval = pdf_test_tt_font(p, img, &n_fonts);
            if (retval == pdc_true)
            {
                if (n_fonts > 1)
                {
                    retval = pdf_check_and_read_ttc(p, fp, fullname, fontname,
                                                    font, n_fonts);
                }
                else
                {
