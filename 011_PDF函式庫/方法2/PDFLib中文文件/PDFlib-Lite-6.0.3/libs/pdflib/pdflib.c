...
/* $Id: pdflib.c,v 1.68.2.17 2006/01/26 19:27:50 kurt Exp $
 *
...
{
#ifdef WIN32
    if (IsBadWritePtr((VOID *) p, (UINT_PTR) sizeof (p)) ||
        p->magic != PDC_MAGIC)
#else
    ...
    {
#ifdef PDC_DEBUG
        printf("*** PDFlib context %p is invalid ***\n", p);
#endif
        return pdc_false;
    }
...
PDF_get_opaque(PDF *p)
{
    ...
    if (pdf__check_context(p))
    {
        ...
        pdf_enter_api2(p, fn, "(p[%p])", (void *) p);
        retval = p->opaque;
