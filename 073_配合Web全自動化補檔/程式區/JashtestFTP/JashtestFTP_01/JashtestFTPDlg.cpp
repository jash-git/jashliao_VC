...
void CJashtestFTPDlg::OnButton2() 
{
	...
	*/
	...
	while(!pf.eof())
	{
		...
		StrFileData=chrdata;
		//blnChangeDir=false;//因為原始路徑已帶目錄所以不用切換
		if(StrFileData!="")
		{
			j=StrFileData.GetLength();
			i=j-1;
			do
			{
				StrBuf=StrFileData.Mid(i,1);
				//if(StrBuf!="/" && blnChangeDir==false)//因為原始路徑已帶目錄所以不用切換
				if(StrBuf!="/") 
				{
					StrSaveFileData=StrBuf+StrSaveFileData;
				}
				else
				{
					break;
			...
			/*//因為原始路徑已帶目錄所以不用切換
			if(blnChangeDir==false)
			{
				StrFTPDir="/";
			}
			intGetDataAns=m_pFtpConnection->SetCurrentDirectory(StrFTPDir);
			*/
			StrSaveFileData="AV\\"+StrSaveFileData;
