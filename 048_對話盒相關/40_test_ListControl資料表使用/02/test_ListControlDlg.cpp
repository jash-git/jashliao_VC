...
void CTest_ListControlDlg::OnClickList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	int1=pp->iItem;
	UpdateData(true);
		m_ListControl.GetItemText(int1,0,data1.GetBuffer(0),10);
		m_v1=data1.GetBuffer(0);
		m_ListControl.GetItemText(int1,1,data2.GetBuffer(0),10);
		m_v2=data2.GetBuffer(0);
		m_ListControl.GetItemText(int1,2,data3.GetBuffer(0),10);
		m_v3=data3.GetBuffer(0);
		m_ListControl.GetItemText(int1,3,data4.GetBuffer(0),10);
		m_v4=data4.GetBuffer(0);
	UpdateData(false);
	m_ListControl.DeleteItem(int1);//刪除
	glngCount--;
	*pResult = 0;
}

void CTest_ListControlDlg::OnButton1() 
{
	// TODO: Add your control notification handler code here
	glngCount++;
	UpdateData(true);
		m_ListControl.InsertItem(glngCount,m_v1);      
		m_ListControl.SetItemText(glngCount,1,m_v2);
		m_ListControl.SetItemText(glngCount,2,m_v3);
		m_ListControl.SetItemText(glngCount,3,m_v4);
	UpdateData(false);
}
