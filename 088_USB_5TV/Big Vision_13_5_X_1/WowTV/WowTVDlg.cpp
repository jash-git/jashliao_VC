...
//*/
	...
	if(gStrDisDriver=="光碟")
	{
		...
		{
			MessageBox("目前本主機無法連上網路，請使用者先移除Big Vision並檢查網路是否正常");
			this->OnCancel(); 
		}
	}
	else
...
void CWowTVDlg::On_click0()
{
	...
	MessageBox("本軟體並無提供滑鼠右鍵功能，還請見諒。","Big Vision");
}
...
void CWowTVDlg::GetSrlNum()
{
	...
	GetVolumeInformation(lpRootPathName,
				...
				lpFileSystemNameBuffer, nFileSystemNameSize);
	strResult.Format("%x",VolumeSerialNumber);
	gStrVolumeSerialNumber=strResult;
	//MessageBox(strResult);
}
...
void CWowTVDlg::OnTimer(UINT nIDEvent) 
{
	...
	switch(nIDEvent)
	{
		...
		case 2:
			...
			{
				KillTimer(2);
				MessageBox("由於Big Vision硬體已移除，因此程式及將關閉，如有不便敬請見諒。");
				this->OnCancel();
			}
			break;
		case 3:
			CWnd* pWnd=FindWindow(NULL,"Big Vision");
			if(pWnd!=NULL)
