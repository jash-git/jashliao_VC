...
CString cszJL_Close						= "JL_Close";////modify by jash
            ...
            /* [size_is][out] */ DISPID* rgDispId)
{
	...
	for(i=0; i<cNames; ++i)
	{
		...
		else if(cszName == cszJL_Close)////modify by jash
		{
			rgDispId[i] = DISPID_JL_Close;////modify by jash
		}
		else
		{
			...
			/* [out] */ UINT* puArgErr)
{
	...
	if(dispIdMember == DISPID_JL_Close)//modify by jash 
	{
		if(wFlags & DISPATCH_PROPERTYGET)
		{
			if(pVarResult != NULL)
