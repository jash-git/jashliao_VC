	...
	if(pointEnd.x!=0 || pointEnd.y!=0)
	{
		...
		for(i=0;i<w;i++)
			for(j=0;j<h;j++)
			{
				...
				G=*q1++;
				R=*q1++;
				*q2++=(int)((R*-0.169)+(G*-0.332)+(B*0.5)+128);
				*q2++=(int)((R*-0.169)+(G*-0.332)+(B*0.5)+128);
				*q2++=(int)((R*-0.169)+(G*-0.332)+(B*0.5)+128);
			}
		dibshow1.SaveBmp("Init_RGB.bmp");
		dibshow2.SaveBmp("Init_G.bmp"); 
	}
	//*/
}
	...
	if(blnStart==true)
	{
		...
		for(i=0;i<w;i++)
		{
			for(j=0;j<h;j++)
			{
				...
				G=*q2++;
				R=*q2++;
				if((B_Init-B)>10||(B_Init-B)<-10)
				{
					intdiffcount++;
				}//if((B_Init-B)>5||(B_Init-B)<-5)
			}//for(j=0;j<h;j++)
		}//for(i=0;i<w;i++)
		if(intdiffcount>=(w*h*0.1) && intdiffcount<(w*h*0.7))
		{
			gintcheck=1;
		}
		else if(intdiffcount>=(w*h*0.7))
		{
			gintcheck=2;
		}
		else
