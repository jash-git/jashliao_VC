...
unsigned char* Cipher1(unsigned char* input);
unsigned char* InvCipher1(unsigned char* input);
void* Cipher2(void* input, int length);
void* InvCipher2(void* input, int length);
...
int ucharToHex(const unsigned char *uch, char *hex)
{
  ...
  while(*uch){
    ...
    high = tmp >> 4;
    low = tmp & 15;
    *hex++ = valueToHexCh(high); //先?高字?
    *hex++ = valueToHexCh(low); //其次?低字?
    uch++;
  }
  *hex = '\0';
  return 0;
...
char valueToHexCh(const int value)
{
  char result = '\0';
  if(value >= 0 && value <= 9){
    result = (char)(value + 48); //48?ascii??的‘0’字符??值
  }
  else if(value >= 10 && value <= 15){
    result = (char)(value - 10 + 65); //?去10?找出其在16?制的偏移量，65?ascii的'A'的字符??值
  }
  else{
    ;
  }
...
int main(int argc, char *argv[])
{
    ...
    AES(key);
    Cipher(mingwen, miwen_hex);
    printf("%s\n",miwen_hex);
    InvCipher(miwen_hex, result);
    printf("%s\n",result);
    getchar();
    return 0;
}
