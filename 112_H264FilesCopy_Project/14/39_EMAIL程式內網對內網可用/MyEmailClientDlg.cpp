	...
	: CDialog(CMyEmailClientDlg::IDD, pParent)
{
	...
	m_To = _T("jashliao@tw-airnet.net");
	m_User = _T("5tv_admin@tw-airnet.net");
	m_Subject = _T("5TV ╰参恨瞶-H264FileCopy岿粇厨");
	//}}AFX_DATA_INIT
...
void CMyEmailClientDlg::OnTimer(UINT nIDEvent) 
{
	...
	CString StrTime;
	StrTime=t1.Format("岿粇丁: %Y/%m/%d %H:%M:%S\n");
	m_Body="";
	m_Body+=StrTime;
	///////////////////////////////////////////
	m_Subject="5TV ╰参恨瞶-H264FileCopy岿粇厨";
	///////////////////////////////////////////////////////
	a1.open("ErrorInfo.txt",ios::in);
	while(!a1.eof())
	{
		StrDataBuf="";
		a1.getline(fg,500);
