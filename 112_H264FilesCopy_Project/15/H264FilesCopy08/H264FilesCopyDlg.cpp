...
CString StrDirS,StrDirD,StrGetListUrl,StrSetWorkIdUrl,StrSetWorkErrorUrl,StrCutImagePath;
...
BOOL CH264FilesCopyDlg::OnInitDialog()
{
	...
	if(blnRead)//第11.0版的功能
	{
		...
		pNodeRoot=pDoc->selectSingleNode("//CutImagePath");//取得Node內容
		pNodeRoot->get_nodeTypedValue(&vtStr);
		StrBuff=vtStr.bstrVal;
		StrCutImagePath=StrBuff;
		StrCutImagePath+="CutImageList.txt";
	}
	else
	{
...
void CH264FilesCopyDlg::Function()
{
	...
	pfCutImageList=fopen("CutImageList.txt","w");//TIMER一進來就建立檔案
	...
	if(blnRead)//第11.0版的功能
	{
		...
		for(i=0;i<j;i++)
		{
			...
			for(k=0;k<l;k++)
			{
				...
				if(k==0)
					fprintf(pfCutImageList,"%s\n",StrDFilePath.GetBuffer(0));//將同一系列第一筆寫到檔案中
	...
	if(pfCutImageList!='\0')//檔案未關閉
	{
		fclose(pfCutImageList);
		pfCutImageList='\0';
		if(j>0)
		{
			CopyFile("CutImageList.txt",StrCutImagePath.GetBuffer(0),false);
		}
