...
void CMatlab_normfitDlg::OnButton2() 
{
	// TODO: Add your control notification handler code here
	////
	mwArray X,alpha,muci,sigmaci,sigmahat,V;
	X=row2mat(1,MarrValue,douFileData);
	double y=0.01;
	alpha=row2mat(1,1,&y);
	...
	V=normfit(&sigmahat,&muci,&sigmaci,X,alpha);
	UpdateData(true);
	V.ExtractData(&m_douMean);
	sigmahat.ExtractData(&m_douSigma);
	UpdateData(false);
	//MessageBox("");
}
