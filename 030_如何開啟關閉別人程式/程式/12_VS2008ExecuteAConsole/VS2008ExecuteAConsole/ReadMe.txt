================================================================================
    MFC 程式庫 : VS2008ExecuteAConsole 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 VS2008ExecuteAConsole 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 VS2008ExecuteAConsole 應用程式的檔案。

VS2008ExecuteAConsole.vcproj
    這是使用應用程式精靈所產生之 VC++ 專案的主要專案檔。 
    它包含產生檔案時的 Visual C++ 版本資訊，以及使用應用程式精靈產生檔案的
    過程中，選取的平台、組態和專案功能的相關資訊。

VS2008ExecuteAConsole.h
    這是應用程式的主要標頭檔。它包含其他專案特定的標頭 (包括 Resource.h)，
    並宣告 CVS2008ExecuteAConsoleApp 應用程式類別。

VS2008ExecuteAConsole.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CVS2008ExecuteAConsoleApp。

VS2008ExecuteAConsole.rc
    這份清單包含了所有程式要使用的 Microsoft Windows 資源。其中包括了儲存
    在 RES 子目錄下的圖示、點陣圖和游標。您可以直接在 Microsoft Visual C++ 
    內編輯這個檔案。您的專案資源在 1028。

res\VS2008ExecuteAConsole.ico
    這個圖示檔是用來做為應用程式的圖示。這個圖示包含在主要資源檔
    VS2008ExecuteAConsole.rc 內。

res\VS2008ExecuteAConsole.rc2
...
VS2008ExecuteAConsoleDlg.h、VS2008ExecuteAConsoleDlg.cpp - 對話方塊
    這些檔案包含您的 CVS2008ExecuteAConsoleDlg 類別。這個類別定義應用程式主
    對話方塊的行為。對話方塊的範本在 VS2008ExecuteAConsole.rc 內，您可以 
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 VS2008ExecuteAConsole.pch 的先行編譯標頭 (PCH) 檔，
...
VS2008ExecuteAConsole.manifest
