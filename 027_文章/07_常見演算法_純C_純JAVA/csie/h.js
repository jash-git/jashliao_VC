...
h.Menu = function()
{
	...
	div.className = 'm';
	div.innerHTML += '<div class="li"><a href="index.html">回到首頁</a></div>';

	var childs = document.body.childNodes;
	for(var i = 0; i < childs.length; i++)
		if(childs[i].className == "a")
		{
			var banner = childs[i].getElementsByTagName('p')[0];
			banner.innerHTML = banner.innerHTML.replace(/: /gi,':<br>').replace(/（/gi,'<br>（').replace(/【/gi,'<br>【');
			div.innerHTML += '<div class="li"><a href="#a' + i + '">' + banner.innerHTML + '</a></div>';
			childs[i].id = 'a' + i;
		}
