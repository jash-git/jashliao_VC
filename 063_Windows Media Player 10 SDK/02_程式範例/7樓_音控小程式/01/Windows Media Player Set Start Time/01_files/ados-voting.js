(function(root) {
	...
	root.adFeedbackCastVote = function(eventId, advertiserId, siteId, zoneId, creativePassId) {
		var url = ['engine.adzerk.net', 'e', eventId, advertiserId, siteId, zoneId, creativePassId, 'e.gif'].join('/');
		var pixel = $("<img src='" + document.location.protocol + '//' + url + "' height=0 width=0 />");
		$('body').append(pixel);
	};

	root.azRegisterExtension('voting', function(id, advertiserId, siteId, zoneId, creativePassId, creativeId) {
		if ($ === undefined) return;
		...
		var castVoteForThisCreative = function(eventId) {
			root.adFeedbackCastVote(eventId, advertiserId, siteId, zoneId, creativePassId);
		};
