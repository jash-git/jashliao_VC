...
void CCreateISOsDlg::OnCreate() 
{
	...
	{
		pf=fopen("miso.bat","w");
		//MISO d:\test.iso -a C:\BigVision_USB5tv_13_5_X_1\* -l BigVision
		if(i<10)
		{
			StrData.Format("miso.exe %s\\000%d.iso -a %s\\* -l BigVision",m_CStrISO.GetBuffer(0),i,m_CStrSou.GetBuffer(0));
		}
		else if(i<100)
		{
			StrData.Format("miso.exe %s\\00%d.iso -a %s\\* -l BigVision",m_CStrISO.GetBuffer(0),i,m_CStrSou.GetBuffer(0));
		}
		else if(i<1000)
		{
			StrData.Format("miso.exe %s\\0%d.iso -a %s\\* -l BigVision",m_CStrISO.GetBuffer(0),i,m_CStrSou.GetBuffer(0));
		}
		else
		{
			StrData.Format("miso.exe %s\\%d.iso -a %s\\* -l BigVision",m_CStrISO.GetBuffer(0),i,m_CStrSou.GetBuffer(0));
		}
